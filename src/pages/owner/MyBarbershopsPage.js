      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 relative animate-fadeIn">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-6">
          <p className="text-sm text-yellow-800">
            <span className="font-semibold">Peringatan:</span> Tindakan ini
            tidak dapat dibatalkan. Semua data termasuk layanan, staf, dan
            riwayat booking akan dihapus permanen.
          </p>
        </div>
...
const NotRegisteredView = () => (
  <div className="text-center max-w-3xl mx-auto mt-10 p-10 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-2xl shadow-lg">
    <h1 className="text-4xl font-extrabold text-white">
      Anda Belum Memiliki Barbershop
    </h1>
    <p className="mt-4 text-lg text-indigo-100">
      Jangkau ribuan pelanggan baru dengan mendaftarkan barbershop Anda.
    </p>
    <Link
      to="/register-barbershop"
      className="inline-block px-8 py-3 mt-8 text-lg font-bold text-indigo-600 bg-white rounded-lg shadow-md hover:bg-gray-100 transition-transform transform hover:scale-105"
    >
      ...
      case "approved":
        return (
          <div className="flex flex-wrap gap-2">
            <Link
              to={`/owner/barbershop/${shop.barbershop_id}/dashboard`}
            ...
            <Link
              to={`/owner/barbershop/${shop.barbershop_id}/services`}
              className="flex items-center px-3 py-1 text-sm text-white bg-blue-600 rounded-md hover:bg-blue-700"
              title="Kelola Layanan"
            >
              <FiScissors />{" "}
              <span className="hidden sm:inline ml-2">Layanan</span>
            </Link>
            <Link
              to={`/owner/barbershop/${shop.barbershop_id}/staff`}
              className="flex items-center px-3 py-1 text-sm text-white bg-gray-700 rounded-md hover:bg-gray-800"
              title="Kelola Staf"
            >
            ...
            <Link
              ...
              className="flex items-center px-3 py-1 text-sm text-white bg-yellow-600 rounded-md hover:bg-yellow-700"
              title="Edit nama/alamat (akan direview admin)"
            >
              Edit Profil
            </Link>
            <button
...
const MyBarbershopsPage = () => {
  ...
  const handleResubmit = async (barbershopId) => {
    if (
      window.confirm(
        "Apakah Anda yakin ingin mengajukan ulang pendaftaran untuk barbershop ini?",
      )
    ) {
      try {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        `Barbershop "${deleteModal.barbershop.name}" berhasil dihapus.`,
      );
      ...
      toast.error(
        err.response?.data?.message ||
          "Gagal menghapus barbershop. Silakan coba lagi.",
      );
      console.error(err);
    } finally {
      setIsDeleting(false);
  ...
  if (loading)
    return <div className="p-8 text-center">Memuat barbershop Anda...</div>;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          ...
          <p className="mt-1 text-gray-600">
            Lihat dan kelola semua barbershop yang Anda miliki.
          </p>
        </div>
        <Link
          to="/register-barbershop"
          className="flex items-center px-4 py-2 mt-4 sm:mt-0 text-white bg-green-600 rounded-md hover:bg-green-700"
        >
